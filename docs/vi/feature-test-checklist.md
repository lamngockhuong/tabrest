...
- [ ] Chrome dọn storage entries (xác minh bằng cài lại sạch: stats reset, không còn snooze/pause/sessions sót).
- [ ] Không còn content script orphan ở các tab đã inject sau khi cài lại.

## 41. Tạm dừng toàn cục

- [ ] Tạm dừng 30 phút / 1 giờ / 2 giờ / "Cho đến khi tôi resume" qua dropdown trên thanh pause trong popup → thanh hiện nút Resume kèm thời gian còn lại (hoặc "Cho đến khi tôi resume").
- [ ] Khi đang tạm dừng, auto-unload (timer, memory, per-tab heap, startup) KHÔNG discard bất kỳ tab nào.
- [ ] Khi đang tạm dừng, các thao tác unload thủ công (Unload Current/Others/Left/Right, context menu, phím tắt, click toolbar) vẫn hoạt động bình thường.
- [ ] Badge trên toolbar hiển thị `❚❚` khi đang tạm dừng.
- [ ] Danh sách tab trong popup ẩn countdown timer của từng tab khi đang tạm dừng.
- [ ] Tạm dừng có thời hạn (30 phút/1 giờ/2 giờ) tự động resume khi hết hạn, không cần thao tác của user; badge và countdown trở lại bình thường.
- [ ] Tạm dừng "Cho đến khi tôi resume" giữ nguyên trạng thái cho đến khi user bấm Resume.
...
- [ ] Side panel: thanh pause phản ánh đúng trạng thái paused/resumed giống popup.
